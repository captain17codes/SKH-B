...
export default function CitizenPortalPage() {
  ...
  return (
    <div className="bg-background text-on-surface font-body-md min-h-screen relative flex flex-col">
      {/* Header */}
      <div className="bg-primary text-on-primary py-8 relative overflow-hidden">
        <div className="absolute -top-16 -right-16 w-64 h-64 bg-primary-container rounded-full blur-3xl opacity-40 pointer-events-none"></div>
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop relative z-10">
          <Link to="/" className="inline-flex items-center gap-2 text-primary-fixed-dim hover:text-on-primary text-sm mb-4 transition-colors">
            <span className="material-symbols-outlined text-[18px]">arrow_back</span> Back to home
          </Link>
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-4xl icon-filled">location_city</span>
            <div>
              <h1 className="text-headline-lg font-headline-lg font-bold">Kopargaon Municipal Council</h1>
      ...
      {/* Main Content */}
      <div className="flex-1 max-w-container-max w-full mx-auto px-margin-mobile md:px-margin-desktop py-10">
        <div className="max-w-2xl mx-auto">
          <TicketForm onSuccess={handleTicketSuccess} />

          {/* Info Section */}
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-5 text-center shadow-sm">
              <span className="material-symbols-outlined text-3xl text-primary mb-2">speed</span>
              <h3 className="font-bold text-sm text-on-surface">AI-Powered Triage</h3>
            ...
            <div className="bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-5 text-center shadow-sm">
              ...
              <h3 className="font-bold text-sm text-on-surface">Status Updates</h3>
              <p className="text-xs text-on-surface-variant mt-1">Track progress with your reference number</p>
            </div>
            <div className="bg-surface-container-lowest border border-outline-variant/20 rounded-xl p-5 text-center shadow-sm">
              <span className="material-symbols-outlined text-3xl text-primary mb-2">verified</span>
              <h3 className="font-bold text-sm text-on-surface">RTS Compliant</h3>
